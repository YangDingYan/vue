...
module.exports = function weexFactory(exports, document) {
  ...
  function initUse(Vue) {
    Vue.use = function (plugin) {
      // * 检查Vue（this）上是否有属性：_installedPlugins = []。存储已安装过的插件
      var installedPlugins = (this._installedPlugins || (this._installedPlugins = []));
      if (installedPlugins.indexOf(plugin) > -1) { //! indexOf检测对象数组时，默认：检查item的引用地址是否相同
        return this
      }

      // additional parameters
      var args = toArray(arguments, 1);
      args.unshift(this);                         //* 组装install方法的执行参数： arguments:[Vue, xx, xx, xx]
      //! install方法： 内部其实是在 Vue构造函数上 挂在一系列 自定义的组件、方法、指令、属性、混入等
